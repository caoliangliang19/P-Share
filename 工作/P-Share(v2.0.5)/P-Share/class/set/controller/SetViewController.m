...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        if (buttonIndex == 1) {
            [[NSNotificationCenter defaultCenter] postNotificationName:KUserExit object:nil];
            [[DataSource shareInstance].temParkingArray removeAllObjects];
            [[DataSource shareInstance].rentStopArray removeAllObjects];
            [[DataSource shareInstance].oilCardListArray removeAllObjects];
            [[DataSource shareInstance].oilOrderListArray removeAllObjects];
            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
            [userDefaults setObject:@"0" forKey:identity];
            ...
            [userDefaults setObject:nil forKey:customer_email];
            [userDefaults setObject:nil forKey:customer_region];
            [userDefaults setObject:nil forKey:customer_job];
            [userDefaults setObject:nil forKey:customer_sex];
            [userDefaults setObject:@"" forKey:customer_head];
            [userDefaults setInteger:0 forKey:customer_selectPark];
            [userDefaults setObject:nil forKey:remindCoupon];
            [userDefaults setObject:@"0" forKey:pay_password];
            [userDefaults setBool:NO forKey:@"couponRedPoint"];
            [userDefaults setObject:nil forKey:@"carNumber"];
            OrderPointModel *model = [OrderPointModel shareOrderPoint];
            model.equity = 0;
            model.monthly = 0;
            model.paker = 0;
...
//    }
    cell.leftImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"setImage%ld",(long)indexPath.row]];
