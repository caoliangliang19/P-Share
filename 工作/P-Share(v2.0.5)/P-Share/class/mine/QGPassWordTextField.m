...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat margin = rect.size.height / 4;
    if (self.selected) {
        self.layer.borderColor = NEWMAIN_COLOR.CGColor;
        
    }else
    {
        self.layer.borderColor = [MyUtil colorWithHexString:@"E0E0E0"].CGColor;
...
//        [[UIColor grayColor]set];
        [[MyUtil colorWithHexString:@"E0E0E0"] set];
        
        CGContextAddRect(context, CGRectMake(i * (width), y, width, height));
        [frameArr addObject:[NSValue valueWithCGRect:CGRectMake(i * (width) + 1.5*margin, y + 1.5*margin, width - 3 * margin, height - 3 * margin)]];
        }
    ...
    if (self.text.length) {
        for (int i = 0; i < self.text.length; i++) {
           [[UIColor blackColor]set];
            CGRect rect = [frameArr[i] CGRectValue];
//            rect.size.height -= 10;
//            rect.size.width  -= 10;
