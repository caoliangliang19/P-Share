...
- (void)initTheInterFace:(NSString *)titleName{
   ...
   //第一个控件
    UILabel *titleNameL = [MyUtil createLabelFrame:CGRectMake(0, FLableHight-30, AlertWidth, 30) title:titleName textColor:[MyUtil colorWithHexString:@"333333"] font:[UIFont systemFontOfSize:15] textAlignment:NSTextAlignmentCenter numberOfLine:1];
    [self addSubview:titleNameL];
   //第三个控件
    
    _titleLable = [MyUtil createLabelFrame:CGRectZero title:self.title textColor:[MyUtil colorWithHexString:@"696969"] font:[UIFont systemFontOfSize:12] textAlignment:NSTextAlignmentLeft numberOfLine:0];
    

    [self addSubview:_titleLable];
    
    UIButton *button1 = [MyUtil createBtnFrame:CGRectMake(AlertWidth-25, 10, 15, 15) title:nil bgImageName:@"payViewCancel" target:self action:@selector(onClickDismiss)];

    //第四个，第五个控件Button
    [self addSubview:button1];
    _button2 = [MyUtil createBtnFrame:CGRectZero title:@"拒绝" bgImageName:nil target:self action:@selector(onRefuse)];
    [_button2 setTitleColor:[MyUtil colorWithHexString:@"aaaaaa"] forState:UIControlStateNormal];
    _button2.titleLabel.font = [UIFont systemFontOfSize:20];
    [self addSubview:_button2];
    _button3 = [MyUtil createBtnFrame:CGRectZero title:@"同意" bgImageName:nil target:self action:@selector(onCertain)];
     [_button3 setTitleColor:[MyUtil colorWithHexString:@"39d5b8"] forState:UIControlStateNormal];
    ...
    _lineView1.backgroundColor = [MyUtil colorWithHexString:@"e0e0e0"];
    [self addSubview:_lineView1];
    //第四个控件
    _lineView2 = [[UIView alloc]initWithFrame:CGRectZero];
    _lineView2.backgroundColor = [MyUtil colorWithHexString:@"e0e0e0"];
    [self addSubview:_lineView2];
    //第六个控件
    _lineView3 = [[UIView alloc]initWithFrame:CGRectZero];
    _lineView3.backgroundColor = [MyUtil colorWithHexString:@"e0e0e0"];
