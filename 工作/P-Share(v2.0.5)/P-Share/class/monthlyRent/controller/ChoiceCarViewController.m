...
- (void)loadCarArrayDataWithBeginIndex:(NSInteger)beginIndex
{
    ...
    NSString *summary = [[NSString stringWithFormat:@"%@%ld%@",userId,(long)_curIndex,MD5_SECRETKEY] MD5];
    
    NSString *url = [NSString stringWithFormat:@"%@/%@/%ld/%@",NewCarList,userId,(long)_curIndex,summary];
    ...
    [RequestModel requestGetCarListWithURL:url WithPage:beginIndex Completion:^(NSArray *dataArray) {
        ...
        [_carArray addObjectsFromArray:dataArray];
...
//    NSDictionary *paramDic = [NSDictionary dictionaryWithObjectsAndKeys:userId,customer_id,indexStr,@"pageindex", nil];
//
//    NSString *urlString = [NewCarList stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.carNameLabel.text = model.carBrand;
