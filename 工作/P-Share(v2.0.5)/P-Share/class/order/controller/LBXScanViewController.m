...
- (void)drawScanView
{
    ...
    [self.view addSubview:_xiangCeBtn];
    [self.view addSubview:_kaiDengBtn];
...
{
    DiscountViewController *scanResultCtrl = [[DiscountViewController alloc] init];
    if (scanResult.strScanned.length > 0) {
        scanResultCtrl.strScan = scanResult.strScanned;
    }else{
        scanResultCtrl.strScan = @"noResult";
    }
    
    [self.navigationController pushViewController:scanResultCtrl animated:YES];
}
