...
//定义服务器地址
#define SERVER_ID               @"139.196.24.16"

#define SERVERERROR             @"数据获取失败"

#define NETWORKINGERROE         @"网络连接失败"

//可一直用的优惠券
#define ISUSECOUPON             @"100201"

//不可使用的优惠券
#define UNUSECOUPON             @"200102"

//MD5密钥
#define MD5_SECRETKEY           @"Boxiang2016"
...
#define GRAY_COLOR ([MyUtil colorWithHexString:@"#eaeaea"])
#define NEWMAIN_COLOR ([MyUtil colorWithHexString:@"#39D5B8"])

#define CUSTOMERMOBILE(STR) ([MyUtil getUseDefault:STR])
 ...
 */
//mbprogress
#define MBPROGRESS_TITLE(title) do{ if([title isKindOfClass:[NSString class]]) {_hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];_hud.mode = MBProgressHUDModeText;_hud.labelText=title;_hud.margin = 10.f;_hud.yOffset = -200.f;_hud.removeFromSuperViewOnHide = YES;[_hud hide:YES afterDelay:2];}}while(0);

#define CHATBTNCLICK QYSessionViewController *sessionVC = [[QYSDK sharedSDK] sessionViewController];sessionVC.sessionTitle = @"口袋停客服";QYCustomUIConfig *custom = [[QYSDK sharedSDK] customUIConfig];NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];NSString *imageString = [userDefaults objectForKey:customer_head];UIImageView *temImgV = [[UIImageView alloc] init];[temImgV sd_setImageWithURL:[NSURL URLWithString:imageString] placeholderImage:[UIImage imageNamed:@"defaultHeaderImage"]];custom.customerHeadImage = temImgV.image;custom.serviceHeadImage = [UIImage imageNamed:@"logoPshare"];[self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor whiteColor]}];sessionVC.navigationItem.hidesBackButton = YES;custom.rightBarButtonItemColorBlackOrWhite = NO;UIButton*btn = [UIButton buttonWithType:UIButtonTypeCustom];btn.frame=CGRectMake(5, 34, 35, 40);[btn setImage:[UIImage imageNamed:@"defaultBack"] forState:UIControlStateNormal];[btn addTarget:self action:@selector(goBackAction)forControlEvents:UIControlEventTouchUpInside];UIBarButtonItem *back=[[UIBarButtonItem alloc]initWithCustomView:btn];[btn setImageEdgeInsets:UIEdgeInsetsMake(10, 0, 10, 20)];sessionVC.navigationItem.leftBarButtonItem=back;QYUserInfo *user = [[QYUserInfo alloc] init];user.userId = @"1234";user.data = @"fay";[[QYSDK sharedSDK] setUserInfo:user];[self.navigationController pushViewController:sessionVC animated:YES];self.navigationController.navigationBarHidden = NO;
...
#define ALLOC_MBPROGRESSHUD_VIEW  _clearBackView = [[UIView alloc] initWithFrame:self.bounds];  _clearBackView.backgroundColor = [UIColor clearColor];  [self addSubview:_clearBackView];  _mbView = [[MBProgressHUD alloc] initWithView:self];  _mbView.opacity = 0.6;  _mbView.labelText = @"加载中...";  _mbView.labelFont = [UIFont systemFontOfSize:13];  _mbView.activityIndicatorColor = MAIN_COLOR;  [_clearBackView addSubview:_mbView];_clearBackView.hidden = YES;
...
//8b1d2391027859b777d9c7d91944a3cd
#define GAODEAPPKEY_test (@"8b1d2391027859b777d9c7d91944a3cd")
...
#define  identity             @"identity"            //用户等级
#define  pay_password         @"pay_password"        //用户钱包是否设置密码 0：没支付密码，1：支付密码

#define  remindCoupon         @"remindCoupon"            //用户优惠券是否提醒

#define  customer_carNumArray @"customer_carNumArray"//存储用户查询车牌号
//#define  indexParkingType     @"index_parking_type"    //首页停车场类型
...
#define  comment_content      @"comment_content"    //评价内容
//正常登陆10 微信01 微博03 qq 04
#define  loginType            @"loginType"          //登录方式
#define  loginopenId               @"openid"             //第三方登录唯一标示
#define  visitorBOOL             @"visitorBOOL"         //游客登陆
