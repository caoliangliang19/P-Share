...
- (id)initWithAnnotation:(id<MAAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _parkStateLabel = [MyUtil createLabelFrame:CGRectZero title:@"" textColor:NEWMAIN_COLOR font:[UIFont boldSystemFontOfSize:16] textAlignment:NSTextAlignmentCenter numberOfLine:1];
        
        [self addSubview:_parkStateLabel];
        __weak typeof(self)weakself = self;
        
        [_parkStateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.mas_equalTo(weakself);
            make.centerY.mas_equalTo(weakself).offset(-4);
...
//        if (selected) {
//            if (_noParkView == nil) {
//                _noParkView = [[CustomNoParkView alloc] initWithFrame:CGRectMake(0, 0, 0,0)];
