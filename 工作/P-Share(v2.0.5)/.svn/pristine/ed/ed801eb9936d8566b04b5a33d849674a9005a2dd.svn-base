...
@interface NewMapHomeVC ()<MAMapViewDelegate,AMapSearchDelegate,CarDetailAlertDelegate>
{
    ...
    NSMutableArray          *_yuZuChanQuanLinTingArray;//用来存放月租、产权、临停
...
@property (nonatomic,assign)  float             nowLatitude,  //保存用户的位置坐标
                                                nowLongitude;
@property (nonatomic,strong)  CarModel          *globalCarModel;//保存选中车辆
...
//刷新代泊预约订单数据
    ...
    if (!_globalCarModel) {
        _globalCarModel = [[CarModel alloc] init];
    }
    _globalCarModel.carNumber = [[NSUserDefaults standardUserDefaults] objectForKey:@"carNumber"];
    
    if (_globalCarModel.carNumber != nil) {
...
#pragma mark -- 找到离lbs最近的停车场
- (ParkingModel *)getNearbyParkingWith:(NSArray *)dataArray
{
    MAMapPoint point1 = MAMapPointForCoordinate(CLLocationCoordinate2DMake(_nowLatitude,_nowLongitude));
    NSMutableArray *array1 = [[NSMutableArray alloc]init];
    for (NSInteger i = 0; i < dataArray.count; i++) {
        ParkingModel *model = dataArray[i];
    ...
    NSString *minuteDistance;

    NSArray *array2 = [self paixu:array1];
    if (array2.count > 0) {
        minuteDistance = array2[0];
        for (NSInteger i = 0; i < dataArray.count; i++) {
            ParkingModel *model = dataArray[i];
            MAMapPoint point2 = MAMapPointForCoordinate(CLLocationCoordinate2DMake([model.parkingLatitude floatValue], [model.parkingLongitude floatValue]));
            CLLocationDistance distance = MAMetersBetweenMapPoints(point1,point2);
            NSString *distanceString = [NSString stringWithFormat:@"%lf",distance];
            if ([[NSString stringWithFormat:@"%@",minuteDistance] isEqualToString: distanceString]) {
                return model;
            }
        }
    }
...
- (void)loadData
{
    ...
    if (!_parkingArray) {
        ...
        _yuZuChanQuanLinTingArray = [NSMutableArray array];
    }
//    获取用户车辆
...
- (void)loadBottomView
{
    MapBottomView *bottomView = [MapBottomView new];
    _bottomView = bottomView;
     __weak typeof(self)ws = self;
    bottomView.showActivityViewBlock = ^()
    {
        [ws showActivityView];
    ...
    [bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.bottom.mas_equalTo(0);
        make.height.mas_equalTo(self.view.mas_width).multipliedBy(0.138);
    }];
}
- (void)addCarClick
{
...
-(void)alertAddCar
{
    UIAlertController *alert = [MyUtil alertController:@"请先添加车辆" Completion:^{
         ...
         [MobClick endEvent:@"CarManage_addCar1ID"];
...
- (void)LoadMapBottomSecondView
{
    ...
    _mapBottomSecondView.payKindViewClick = ^(NewParkingModel *model,PayKindView *payKindView){
        if (!ws.isVisitor) {
         ...
         [MobClick endEvent:@"AtOnceGetMoneyID"];
        if (payKindView.payKind == PayKindNoCar) {
            [MobClick endEvent:@"CarManage_addCar2ID"];
//
            //前往车辆管理界面
            ...
            NewMonthlyRentSearchVC *month = [[NewMonthlyRentSearchVC alloc] init];
            month.orderType = @"13";
            [ws.navigationController pushViewController:month animated:YES];
            
        }else if (payKindView.payKind == PayKindLinTing){
            
            NewTemParkingVC *newTemVC = [[NewTemParkingVC alloc] init];
            [ws.navigationController pushViewController:newTemVC animated:YES];
...
- (void)loadMapSearchView
{
    if (!_mapSearchView) {
        ...
        mapSearchView.goToUserCenter = ^(){
            ...
            [MobClick endEvent:@"SelfCentreID"];
...
- (void)rightBtnClick:(UIButton *)btn
{
    ...
    if (btn.tag == 1) {
        ...
        [MobClick endEvent:@"CarManagerID"];
        CarListViewController *CarList = [[CarListViewController alloc]init];
        CarList.pushType = @"WashCar";
        CarList.passOnCarNumber = ^(NSString *carNumber){
            NSString *str = [carNumber substringFromIndex:4];
            _carNameL.text = [NSString stringWithFormat:@"*%@",str];
            _globalCarModel.carNumber = carNumber;
            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
            [userDefaults setObject:carNumber forKey:@"carNumber"];
...
//            [_mapview removeAnnotations:_mapview.annotations];
            [MobClick endEvent:@"LocationFamilyID"];
            MACoordinateRegion region;
            region.span = MACoordinateSpanMake(0.01, 0.01);
...
//            [self addAnnotationWithCooordinate];
        ...
        [MobClick endEvent:@"CollectListID"];
...
- (void)loadRightBtn
{
    if (!_carBtn) {
        ...
        [_carNameL mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(_carBtn.mas_bottom).offset(-3);
...
- (void)onClock{
    ParkingModel *model = _globalParkModel;
    NSString *summary = [[NSString stringWithFormat:@"%@%@",model.parkingId,MD5_SECRETKEY] MD5];
    NSString *url = [NSString stringWithFormat:@"%@/%@/%@",queryCharge,model.parkingId,summary];
    BEGIN_MBPROGRESSHUD
    [RequestModel requestGetQueryChargeWithURL:url Completion:^(NSDictionary *dict) {
        ...
        if (array.count == 2) {
            ...
            clearCar.parkingModel = model;
            clearCar.dict = dict;
            if ([self.carNameL.text isEqualToString:@"_ _"]) {
                clearCar.carNumber = @" ";
            }else{
                clearCar.carNumber = _globalCarModel.carNumber;
            }
            [self.navigationController pushViewController:clearCar animated:YES];
        }else{
            UIAlertController *alert = [MyUtil alertController:@"此车场暂未开通" Completion:^{
...
- (void)showActivityView
{
    ...
    _mapBottomSecondView.hidden = YES;
    [UIView animateWithDuration:0.3 animations:^{
        

        
        [_mapActivityView mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)mapView:(MAMapView *)mapView didFailToLocateUserWithError:(NSError *)error
{
    ...
    switch([error code]) {
        case kCLErrorDenied:
            ...
            if (num == 1) {
                ALERT_VIEW(@"请去设置页面找到口袋停,打开位置服务权限");
                _alert = nil;
                num = 0;
...
- (void)mapView:(MAMapView *)mapView didUpdateUserLocation:(MAUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation
{
    ...
    if (_nowLatitude < 2 && _nowLongitude < 2) {
        ...
        {
            _isMove = YES;
            MyLog(@"当用户坐标发生改变时，重新获取停车场数组 坐标：%f %f",_mapview.userLocation.coordinate.latitude,_mapview.userLocation.coordinate.longitude);
            MACoordinateRegion region;
            region.span = MACoordinateSpanMake(0.01, 0.01);
            region.center =  CLLocationCoordinate2DMake(_nowLatitude,_nowLongitude);
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [RequestModel requestGetPhoneNumWithURL:url WithDic:nil Completion:^(NSDictionary *dict) {
            if ([dict[@"errorNum"] isEqualToString:@"0"])
            {
                ...
                if (!_nearbyParkModel) {
                    _nearbyParkModel = [self getNearbyParkingWith:_parkingArray];
                }
...
- (void)addAnnotationWithCooordinate
{
    ...
    if (_againIndex == 0) {
        ...
        [self performSelector:@selector(againClick) withObject:self afterDelay:0.5];
    }
...
- (void)againClick{
    [_mapview removeAnnotations:_mapview.annotations];
    MACoordinateRegion region1;
    region1.span = MACoordinateSpanMake(0.01, 0.01);
    region1.center.latitude = _nowLatitude;
    region1.center.longitude = _nowLongitude;
    _mapview.region = region1;
    [self addAnnotationWithCooordinate];
    _againIndex = 1;
}
...
- (void)setAnnotationView:(FayAnnotationView *)fayView
{
    ...
    if (annotation) {
        ...
        if ([annotation.model.isCooperate intValue] == 2) {
            ...
            fayView.portrait = [UIImage imageNamed:@"blueAnnotationBig_v2"];
        ...
        fayView.goToParkDetail = ^()
        {
            ...
            [MobClick endEvent:@"GoInCarDetailID"];
...
#pragma 添加完大头针调用的方法,设置大头针的红绿黑颜色(大头针问题)
//- (void)mapView:(MAMapView *)mapView didAddAnnotationViews:(NSArray *)views;
//{
//    
//    for (int i=0; i < views.count; i++) {
//        if (![views[i] isKindOfClass:[FayAnnotationView class]]) {
//            return ;
//        }
//        FayAnnotationView *fayView = (FayAnnotationView *)views[i];
//        ParkingModel *model =_homeArray.dataArray[0];
//
//        FayPointAnnotation *annotation = (FayPointAnnotation *)fayView.annotation;
//        if ([model.parkingId isEqualToString:annotation.model.parkingId]) {
//            
//            fayView.image = [UIImage imageNamed:@"homeAnnotation_v2"];
//            
//        }else{
//            if ([annotation.model.isCooperate intValue] == 2) {
//                fayView.image = [UIImage imageNamed:@"blueAnnotationBig_v2"];
//            }else
//            {
//                fayView.image = [UIImage imageNamed:@"greenAnnotation_v2"];
//            }
//            
//        }
//        fayView.goToParkDetail = ^()
...
#pragma  mark -- 获取临停订单
- (void)loadLinTingOrderWith:(CarModel *)carModel
{
    if (carModel.carNumber.length < 5) {
        return;
    }
    
    [_yuZuChanQuanLinTingArray removeAllObjects];
    NSString *summary = [[NSString stringWithFormat:@"%@%@%@",[MyUtil getCustomId],carModel.carNumber,MD5_SECRETKEY] MD5];
    NSString *url = [NSString stringWithFormat:@"%@/%@/%@/%@",TemParkingOrderList,[MyUtil getCustomId],carModel.carNumber,summary];
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [RequestModel requestTemParkingOrderWithURL:url WithTag:@"cars" Completion:^(NSArray *resultArray) {
          
            if (resultArray.count>0) {
                ParkingModel *model = [[ParkingModel alloc] init];
                TemParkingListModel *temModel = [resultArray objectAtIndex:0];
                ...
                [_yuZuChanQuanLinTingArray addObject:dic];
            }
...
#pragma mark -- 获取月租产权
- (void)loadMonthlyEquityWith:(CarModel *)carModel
{
    
    if (_isVisitor) {//不是游客
        
        NSString *summary = [[NSString stringWithFormat:@"%@%@%@%@",[MyUtil getCustomId],carModel.carNumber,@"2.0.0",MD5_SECRETKEY] MD5];
        NSString *url = [NSString stringWithFormat:@"%@/%@/%@/%@/%@",getMonthlyEquity,[MyUtil getCustomId],carModel.carNumber,@"2.0.0",summary];
       
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [RequestModel requestGetMonthlyEquityWith:url Completion:^(NSDictionary *dataDic)
            {
                for (int i=0; i<2; i++) {
                    if (i == 0) {
                        NSString *parkingId = dataDic[@"equity"][@"parkingId"];
                        if (parkingId.length >0) {
                            NewParkingModel *model = [NewParkingModel shareNewParkingModel:dataDic[@"equity"]];
                            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:model,@"equity", nil];
                            [_yuZuChanQuanLinTingArray addObject:dic];
                        }
                    }else
                    {
                        NSString *parkingId = dataDic[@"monthly"][@"parkingId"];
                        if (parkingId.length >0) {
                            NewParkingModel *model = [NewParkingModel shareNewParkingModel:dataDic[@"monthly"]];

                            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:model,@"monthly", nil];
                            [_yuZuChanQuanLinTingArray addObject:dic];
                        }
                    }
                }
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    END_MBPROGRESSHUD
                    _mapBottomSecondView.dataArray = _yuZuChanQuanLinTingArray;
                });
...
- (void)loadCarArray
{
    ...
    [_selfView getUserCarArrayWhenCompetion:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([MyUtil isBlankString:carNumber] == YES) {
        ...
        _mapBottomSecondView.dataArray = @[@""];
        END_MBPROGRESSHUD;
    }];
}
#pragma mark -- 获取收藏车场
...
//        [ws addAnnotationWithCooordinate];
...
- (void)getHomePark
{
    ...
    homeArray.dataArray = [NSMutableArray array];
    if (homeArray.dataArray.count == 0) {
        ParkingModel *model = [[ParkingModel alloc] init];
        [homeArray.dataArray addObject:model];
    }
    ...
    [RequestModel requestGetParkingListWithURL:homeURL WithTag:@"4" Completion:^(NSMutableArray *resultArray) {
        for (ParkingModel *model in resultArray) {
            ...
            if ([model.homeParkingType isEqualToString:@"1"]) {
                
                [homeArray.dataArray replaceObjectAtIndex:0 withObject:model];
                
            }else if ([model.homeParkingType isEqualToString:@"2"]){
//                公司暂不处理
            }
        }
...
//        给_globalParkModel赋值
        ...
        if ([annotation.model.isCooperate intValue]== 1) {//非合作
            ...
            [MobClick endEvent:@"ClickRedAnnotationID"];
        }else
        {
            //        获取气泡内容
            [self getBubbleInfoWith: annotation.model Completion:^(ParkingModel *newParkModel) {
                [MobClick endEvent:@"ClickBlueAnnotationID"];
    ...
    [RequestModel IsExitParkingWithURL:url Completion:^(NSMutableArray *array) {
        ...
        if ([[array objectAtIndex:0] isEqualToString:@"1"]) {
            ...
            [UIView animateWithDuration: 0.3 delay: 0 usingSpringWithDamping: 0.6 initialSpringVelocity: 10 options: 0 animations: ^{
                ...
                _globalParkModel = _nearbyParkModel;
...
- (NSString *)getDistanceWithParkModel:(ParkingModel *)model
{
    ...
    if (temDistance.length >3) {
        return [NSString stringWithFormat:@"%@千米",temDistance];
    }else
    {
