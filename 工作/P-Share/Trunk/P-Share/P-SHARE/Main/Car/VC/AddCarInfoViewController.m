...
{
    UIBarButtonItem *rightBarItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"camera"] style:UIBarButtonItemStylePlain target:self action:@selector(ORCRecognize)];
...
- (void)loadKayBoard
{
    CLLTextView *carNumTextView = [[CLLTextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 50)];
...
- (IBAction)commitBtnClick:(id)sender {
    ...
    [NetWorkEngine postRequestUse:(self) WithURL:APP_URL(bindingcar) WithDic:paramDic needEncryption:YES needAlert:YES showHud:YES success:^(NSURLSessionDataTask *task, id responseObject) {
...
{
//    
//    NSString *appcode = @"601bd6d23d7d4fadac5688d6a4c77892";
////    行驶证
//    NSString *host = @"https://dm-53.data.aliyun.com";
//    NSString *path = @"/rest/160601/ocr/ocr_vehicle.json";
////驾驶证、
////    NSString *host = @"https://dm-52.data.aliyun.com";
...
//    NSString *bodys = [NSString stringWithFormat:@"{\"inputs\":[{\"image\":{\"dataType\":50,\"dataValue\":\"%@\"}}]}",result];
//    
...
//    return;
    ...
    NSString * postPath = @"/rest/160601/ocr/ocr_driver_license.json";
    NSData *imageData = UIImageJPEGRepresentation(image, 0.5);
    NSString *result = [imageData base64EncodedString];
