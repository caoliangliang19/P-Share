...
- (void)loadMonthlyEquity
{
    ...
    [self ActivityViewShow];
    [NetWorkEngine postRequestUse:(self) WithURL:APP_URL(getMonthlyEquity) WithDic:dic needEncryption:YES needAlert:NO showHud:NO success:^(NSURLSessionDataTask *task, id responseObject) {
        [self ActivityViewHidden];
        MyLog(@"%@",responseObject);
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
