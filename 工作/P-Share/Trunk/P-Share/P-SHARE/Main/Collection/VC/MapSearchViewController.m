...
- (void)createHeadView
{
    ...
    if ([GroupManage shareGroupManages].homeParking) {
        [setHomeButton setTitle:[GroupManage shareGroupManages].homeParking.parkingName forState:(UIControlStateNormal)];
    }else{
        [setHomeButton setTitle:@"设置家的地址" forState:(UIControlStateNormal)];

    }
    setHomeButton.backgroundColor = [UIColor whiteColor];
    ...
    [setHomeButton setTitleColor:[UIColor grayColor] forState:(UIControlStateNormal)];
...
{
    [NetWorkEngine postRequestUse:(self) WithURL:urlStr WithDic:dic needEncryption:YES needAlert:YES showHud:YES  success:^(NSURLSessionDataTask *task, id responseObject) {
