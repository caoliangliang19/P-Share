...
- (void)viewDidLoad {
    ...
    if ([UtilTool isBlankString:self.pingZhengModel.appointmentDate] == NO) {
        _ParkingModel = [[OrderModel alloc]init];
        [_ParkingModel setValuesForKeysWithDictionary:self.pingZhengModel.startEndTime];
    }
  
   //未付款，已付款，已取消
    if ([self.pingZhengModel.voucherStatus integerValue] == 0) {
        [self myanimation];
    }else if ([self.pingZhengModel.voucherStatus integerValue] == 1){
         self.imageV.image = [UIImage imageNamed:@"alreadyused"];
    }else if ([self.pingZhengModel.voucherStatus integerValue] == 2){
...
- (void)dealloc{
    if ([self.type isEqualToString:@"left"]) {
       [[NSNotificationCenter defaultCenter]postNotificationName:@"REFRESHPINGZ" object:nil];
    }
     [[UIScreen mainScreen] setBrightness:_value];
